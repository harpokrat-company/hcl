...
HCL::PublicKey::PublicKey(__mpz_struct modulus, __mpz_struct public_key) :
    PublicKey() {
  modulus_ = modulus;
...
std::string HCL::PublicKey::SerializeContent() const {
  ...
  serialized_header.fields_sizes[2] = mpz_sizeinbase(&(this->public_key_), 2) / sizeof(char);
  ...
  mpz_export(serialized_public_key, nullptr, 1, sizeof(char), 1, 0, &public_key_);
